...
# contents,labels = read_file_seg(filename)
def build_vocab(filename,vocab_dir = "../data/vocab_list.txt",vocab_size=4000):
    start_time=time.time()
    contents,labels = read_file_seg(filename)
    ...
    with codecs.open(vocab_dir,'w',encoding='utf-8') as f:
        f.write('\n'.join(words)+'\n')
    end_time=time.time()
    print('The vocabulary build has been completed,vocab_dir:',vocab_dir,', cost ',end_time-start_time,'s')
...
# x,y = process_file(filename,word_to_id,cat_to_id,max_length=600)

def batch_iter(x,y,batch_size=4):
    data_len=len(x)
...
def train_word2vec(filename,vector_word_filename):
    t1 = time.time()
    sentences,_ = read_file_seg(filename)
